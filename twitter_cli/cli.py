...
# =============================================================================
# UNIQUE FEATURE 1 — AI Feed Digest
# =============================================================================

@cli.command()
@click.option("-t", "--type", "feed_type", default="for-you",
              type=click.Choice(["for-you", "following"]),
              help="Feed type to digest (default: for-you)")
@click.option("--max", "max_count", default=40, show_default=True,
              help="Number of tweets to fetch before summarising")
@click.option("--filter", "apply_filter", is_flag=True, default=False,
              help="Apply engagement scoring before summarising")
@click.option("--api-key", default=None, envvar="ANTHROPIC_API_KEY",
              help="Anthropic API key (or set ANTHROPIC_API_KEY env var)")
def digest(feed_type, max_count, apply_filter, api_key):
    # type: (str, int, bool, str) -> None
    """Summarise your Twitter feed into a daily AI digest.
    ...
    client = TwitterClient(cookies=cookies, config=config)

    with console.status(f"[bold cyan]Fetching {feed_type} feed ({max_count} tweets)..."):
        tweets = client.get_home_timeline(feed_type=feed_type, count=max_count)

    if not tweets:
        console.print("[yellow]No tweets found — nothing to digest.[/yellow]")
        return

    if apply_filter:
        tweets = filter_tweets(tweets, config.get("filter", {}))

    console.print(f"[dim]Fetched {len(tweets)} tweets. Generating AI digest...[/dim]")

    try:
        digest_text = generate_digest(tweets, api_key=api_key)
    except RuntimeError as exc:
    ...
    console.print(Panel(
        ...
        title="[bold cyan]Daily Feed Digest[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    ))
...
# =============================================================================
# UNIQUE FEATURE 2 — Local Archive
# =============================================================================

@cli.group()
def archive():
    # type: () -> None
    """Manage your local tweet archive (SQLite, stored offline).

    \b
    Examples:
        twitter archive stats
        twitter archive search "AI agent"
...
def archive_stats_cmd():
    # type: () -> None
    """Show archive size, date range, and top authors."""
    from .archive import archive_stats
    from rich.table import Table

    stats = archive_stats()
    console.print(f"\n[bold]Archive location:[/bold] {stats['db_path']}")
    console.print(f"  Total tweets : [cyan]{stats['total_tweets']:,}[/cyan]")
    console.print(f"  Oldest tweet : {stats['oldest_tweet'] or 'n/a'}")
    console.print(f"  Newest tweet : {stats['newest_tweet'] or 'n/a'}")

    if stats["top_authors"]:
        tbl = Table(title="Top Authors in Archive", show_header=True)
        tbl.add_column("@handle")
        tbl.add_column("saved tweets", justify="right")
        for row in stats["top_authors"]:
            tbl.add_row(row["screen_name"], str(row["n"]))
        console.print(tbl)


@archive.command(name="search")
@click.argument("query")
@click.option("--max", "limit", default=50, show_default=True, help="Max results")
@structured_output_options
def archive_search_cmd(query, limit, as_json, as_yaml):
    # type: (str, int, bool, bool) -> None
    """Full-text search over your saved tweets (works offline).

    \b
    Examples:
        twitter archive search "machine learning"
        twitter archive search "python OR rust" --max 20
        twitter archive search "GPT" --json
    """
    from .archive import search_archive
    from rich.table import Table

    results = search_archive(query, limit=limit)
    ...
    if as_json or as_yaml:
        emit_structured(results, as_json=as_json, as_yaml=as_yaml)
        return

    tbl = Table(title=f'Archive search: "{query}"', show_header=True, show_lines=False)
    tbl.add_column("#", width=4, justify="right")
    tbl.add_column("@handle", width=20)
    tbl.add_column("tweet")
    tbl.add_column("likes", width=7, justify="right")
    tbl.add_column("source", width=14)

    for i, r in enumerate(results, 1):
        tbl.add_row(str(i), r["screen_name"], r["text"][:160],
                    str(r["likes"]), r.get("source", ""))
    console.print(tbl)


@archive.command(name="list")
@structured_output_options
def archive_list_cmd(as_json, as_yaml):
    # type: (bool, bool) -> None
    """List recent archive fetch sessions."""
    from .archive import list_sessions
    from rich.table import Table

    sessions = list_sessions()
    if not sessions:
        console.print("[yellow]Archive is empty. Use --save with feed/search to populate it.[/yellow]")
        return

    if as_json or as_yaml:
        emit_structured(sessions, as_json=as_json, as_yaml=as_yaml)
        return

    tbl = Table(title="Archive Sessions", show_header=True)
    tbl.add_column("source")
    tbl.add_column("date")
    tbl.add_column("tweets", justify="right")
    for s in sessions:
        tbl.add_row(s["source"], (s["last_fetch"] or "")[:10], str(s["tweet_count"]))
    console.print(tbl)


@archive.command(name="clear")
@click.confirmation_option(prompt="Delete ALL archived tweets? This cannot be undone.")
def archive_clear_cmd():
    # type: () -> None
    """Delete all archived tweets."""
    from .archive import clear_archive
    n = clear_archive()
    console.print(f"[green]Deleted {n:,} tweets from archive.[/green]")


# =============================================================================
# UNIQUE FEATURE 3 — Tweet Scheduler
# =============================================================================

@cli.group()
def schedule():
    # type: () -> None
    """Schedule tweets to post at a future time (no third-party service needed).

    Jobs are stored in ~/.twitter_cli/schedule.json.
    Run `twitter schedule run` in the background to send them automatically.

    \b
    Examples:
        twitter schedule add "Good morning!" --at "2026-03-21 09:00"
        twitter schedule add "Post this later" --in 2h30m
...
@click.option("--at", "at_time", default=None,
              help="Absolute time e.g. '2026-03-21 09:00'")
@click.option("--in", "in_time", default=None,
              help="Relative offset e.g. '2h', '30m', '1h30m'")
@click.option("--reply-to", default=None, help="Tweet ID to reply to")
def schedule_add(text, at_time, in_time, reply_to):
    # type: (str, str, str, str) -> None
    """Schedule a tweet TEXT for future posting.

    \b
    Examples:
        twitter schedule add "Hello world!" --at "2026-03-21 09:00"
        twitter schedule add "Reminder post" --in 2h
        twitter schedule add "Part 2" --in 30m --reply-to 1234567890
    """
    from .scheduler import add_job, parse_schedule_time

    if bool(at_time) == bool(in_time):
        console.print("[red]Provide exactly one of --at or --in.[/red]")
        raise SystemExit(1)

    raw = at_time or in_time
    try:
        scheduled = parse_schedule_time(raw)
    except ValueError as exc:
        console.print(f"[red]{exc}[/red]")
        raise SystemExit(1)

    job = add_job(text, scheduled, reply_to=reply_to)
    console.print(
        f"[green]Scheduled[/green] job [bold]{job['id']}[/bold] "
        f"for [cyan]{scheduled.strftime('%Y-%m-%d %H:%M UTC')}[/cyan]"
    )
...
@click.option("--all", "show_all", is_flag=True, default=False,
              help="Include sent and failed jobs too")
@structured_output_options
def schedule_list(show_all, as_json, as_yaml):
    # type: (bool, bool, bool) -> None
    """List your scheduled tweet jobs."""
    from .scheduler import list_jobs
    from rich.table import Table

    jobs = list_jobs(status=None if show_all else "pending")
    if not jobs:
        console.print("[yellow]No scheduled jobs found.[/yellow]")
        return

    if as_json or as_yaml:
        emit_structured(jobs, as_json=as_json, as_yaml=as_yaml)
        return

    tbl = Table(title="Scheduled Tweets", show_header=True)
    tbl.add_column("ID", width=10)
    tbl.add_column("scheduled (UTC)", width=18)
    tbl.add_column("status", width=9)
    tbl.add_column("text")

    colors = {"pending": "cyan", "sent": "green", "failed": "red"}
    for j in jobs:
        c = colors.get(j["status"], "white")
        tbl.add_row(
            j["id"],
            (j["scheduled"] or "")[:16].replace("T", " "),
            f"[{c}]{j['status']}[/{c}]",
            j["text"][:80],
        )
    console.print(tbl)
...
def schedule_cancel(job_id):
    # type: (str) -> None
    """Cancel a pending scheduled job by its JOB_ID."""
    from .scheduler import cancel_job

    if cancel_job(job_id):
        console.print(f"[green]Cancelled job {job_id}.[/green]")
    else:
        console.print(f"[yellow]Job '{job_id}' not found or already sent/failed.[/yellow]")


@schedule.command(name="run")
@click.option("--interval", default=30, show_default=True,
              help="Seconds between checks")
def schedule_run(interval):
    # type: (int) -> None
    """Start the scheduler daemon — checks for due jobs and posts them.

    Run this in a background terminal or with:

    \b
        start /B twitter schedule run        (Windows)
        nohup twitter schedule run &         (Mac/Linux)
    """
    import logging
    from .scheduler import run_daemon

    logging.basicConfig(level=logging.INFO, format="%(asctime)s  %(message)s")
    console.print("[bold cyan]Scheduler daemon running.[/bold cyan] Press Ctrl+C to stop.\n")
    ...
    def _post(text, reply_to):
        client.post_tweet(text, reply_to_id=reply_to)

    try:
        run_daemon(_post, poll_interval=interval)
